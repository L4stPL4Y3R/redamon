...
>
> Scope covers `supply_chain_scan/`, `supply_chain_common/` (shared logic), and
...
- **NEVER** rely on `supply_chain_common` being importable in a spawned scan
  ...
  [redamon.sh:2228](../redamon.sh#L2228)).
...
malicious/vulnerable package detector. `supply_chain_common/` is the shared
library (bind-mounted, not baked); `supply_chain_analyzer/` dispatches; the scan
