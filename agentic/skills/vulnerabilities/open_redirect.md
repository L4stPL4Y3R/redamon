...
Caveat: redamon.* (proxy_brain) prove the server emitted the redirect only. Address-bar confirmation (where the browser actually navigates) still needs `execute_playwright`.
